# ComicLab

ComicLab 是一个面向 Android 的本地漫画和电子书阅读器，主要用于浏览手机存储中的漫画文件与电子书，并阅读 ZIP、CBZ、RAR、CBR、7Z、CB7、PDF 及无 DRM MOBI 文件。

项目当前重点是本地文件浏览、漫画封面预览、漫画预览、高清阅读和阅读进度管理。

（由于作者此前使用的漫画阅读器广告越来越多，要么不更新某些好用的功能，决定还是自己写一个算了，能满足自己的需求的同时，还**没有广告**）

**如果您也在使用，并且存在需求和问题，请反馈给作者（比心）**

安装包下载地址：

[download](https://github.com/Cillebokin/ComicLab/releases)

## 支持格式

| 类型 | 格式 |
| --- | --- |
| 漫画       | pdf                            |
| 漫画压缩包 | ZIP, CBZ, RAR, CBR, 7Z, CB7    |
| 电子书     | MOBI（无 DRM）                 |
| 图片 | JPG, JPEG, PNG, WEBP, GIF, BMP |

## 运行环境

- Android 13 或更高版本
- 需要授予“管理所有文件”权限

## 版本更新内容

详细更新内容见：

```text
版本说明.txt
```

## 作者

Cillebokin / swcillebokin@gmail.com

项目地址：

```text
https://github.com/Cillebokin/ComicLab
```
